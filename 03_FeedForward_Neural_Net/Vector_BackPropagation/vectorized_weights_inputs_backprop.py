...
class FF_MultiClass_InputWeightVectorised:
    ...
    def grad(self, X, Y):
        ...
        self.dA1 = np.multiply(self.dH1, self.grad_sigmoid(
            self.H1))  # (N, 2) .* (N, 2) -> (N, 2)
    ...
    def fit(self, X, Y, epochs=1, learning_rate=1, display_loss=False):
        ...
        if display_loss:
            ...
            plt.ylabel('Log Loss')
            plt.savefig(
                'Dev-Training-DL/Exercises/02_FeedForward_Neural_Net/Vector_BackPropagation/Loss_Graphs/Vector_Inputs_Weights_Backprop.png')
            plt.show()
